...
from model_example import build_transformer
...
def run_validations(model, val_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_example = 2):
    model.eval()
    count = 0
...
def get_ds(config):
    ds_raw = load_dataset('opus_books',f'{config["lang_src"]}-{config["lang_tgt"]}', split = "train") # for the download of dataset

    # Build tokenizer
    tokenizer_src = get_or_build_tokenizer(config, ds_raw, config["lang_src"])
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, config["lang_tgt"])

    # keep 90% for training other for validation
...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        run_validations(model, val_dataloader, tokenizer_src, tokenizer_tgt, config['seq_len'], device, lambda msg : batch_iterator.write(msg),global_step, writer)
