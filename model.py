...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention: MultiHeadAttention, feed_forward: FeedForwardBlock, dropout:float) -> None:
        super().__init__()
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, features: int, self_attention: MultiHeadAttention, cross_attention: MultiHeadAttention, feed_forward: FeedForwardBlock, dropout: float) -> None:
        super().__init__()
        self.self_attention_block = self_attention
...
def build_transformer(src_vocab_size, tgt_vocab_size, src_seq_len: int, tgt_seq_len: int, d_model: int=512, N: int=6, H:int=8, dropout=0.1, d_ff:int=2048):
    ...
    for _ in range(N):
        ...
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        decoder_block = DecoderBlock(d_model, decoder_self_attention_block, decoder_cross_attemtion_block, feed_forward_block, dropout)
